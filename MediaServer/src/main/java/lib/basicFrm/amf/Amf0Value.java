...
{
	private static AnyLogger log = AnyLogger.getInstance();
	...
	public static void encode(final ChannelBuffer out, final Object value) {
		...
		log.trace("amfValue >> " + toString(type, value));
			...
			case OBJECT:
				...
				while (in.readable()) {
					...
					if (Arrays.equals(endMarker, OBJECT_END_MARKER)) {
						in.skipBytes(3);
						log.trace("end MAP / OBJECT, found object end marker [000009]");
						break;
					}
					if(count > 0 && i++ == count) {
						log.trace("stopping map decode after reaching count : " + count);
						break;
					}
